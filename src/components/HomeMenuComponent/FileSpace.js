...
const FileSpace = ({ file }) => {
  ...
  return (
    <Flex background={useColorModeValue('#282C34','#FAF7F7')} height={'20%'} width={'77%'} borderRightRadius={'3xl'} mt={'1rem'}>
      <Link onFocus={() => playSound()} display={'flex'} alignItems={'center'} alignContent={'center'} pl={'0.5rem'} as={RouteLink} to={`/e/${file.folder_id_folder}`} state={{ id_file: file.id_file }} w='100%'>
