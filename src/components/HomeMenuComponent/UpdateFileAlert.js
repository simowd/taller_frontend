...
const UpdateFileAlert = ({ isOpen, onClose, project, projects }) => {
  ...
  const yup_update_folder = Yup.object({
    folder_name: Yup.string().max(255, stringTranslate('forms.max_length', { length: 255 })).notOneOf((projects.map((folder) => folder.id_folder !== project.id_folder ? folder.folder_name : null)), stringTranslate('forms.repeated')).required(stringTranslate('forms.required')),
